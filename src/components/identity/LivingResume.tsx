...
import { resumeService, ResumeSection, PersonalInfo, Experience, Education, Skill, Project, Achievement } from '@/lib/resume-service';
import { 
  User, 
  Users,
  Mail, 
...
// Interfaces are now imported from resume-service.ts
...
const LivingResume = () => {
  ...
  useEffect(() => {
    const loadResumeData = async () => {
      ...
      try {
        ...
        const { data: sections, error: sectionsError } = await resumeService.getResumeSections(user.id);
        ...
        if (sections && !sectionsError) {
          ...
          sections.forEach(section => {
            switch (section.type) {
              case 'personal':
                setPersonalInfo(section.content);
                break;
              case 'experience':
                setExperiences(section.content);
                break;
              case 'education':
                setEducation(section.content);
                break;
              case 'skills':
                setSkills(section.content);
                break;
              case 'projects':
                setProjects(section.content);
                break;
              case 'achievements':
                setAchievements(section.content);
                break;
            }
          });
        } else {
          console.warn('Using mock data due to API error:', sectionsError);
          // Keep using mock data
        }

        // Auto-update from other sources
        await resumeService.autoUpdateFromProjects(user.id);
        await resumeService.autoUpdateFromLearning(user.id);
        await resumeService.autoUpdateFromAchievements(user.id);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Export to JSON first (PDF generation would require additional library)
      const { data: resumeData, error } = await resumeService.exportResumeToJSON(user.id);
      ...
      if (resumeData && !error) {
        ...
        a.href = url;
        a.download = `resume-${user.id}.json`;
  ...
  const handleToggleVisibility = async (section: string, isVisible: boolean) => {
    if (!user) return;
    
    try {
      await resumeService.updateResumeSectionVisibility(user.id, section, isVisible);
    } catch (error) {
