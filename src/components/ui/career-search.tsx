    ...
    const loadFilterOptions = async () => {
      try {
        const [cats, inds, sks] = await Promise.all([
          careerAPIService.getCategories().catch(err => {
            console.warn('Failed to load categories, using defaults:', err.message);
            return [];
          }),
          careerAPIService.getIndustries().catch(err => {
            console.warn('Failed to load industries, using defaults:', err.message);
            return [];
          }),
          careerAPIService.getSkills().catch(err => {
            console.warn('Failed to load skills, using defaults:', err.message);
            return [];
          })
        ]);
        setCategories(cats);
        ...
        // Set default empty arrays to prevent UI issues
        setCategories([]);
        setIndustries([]);
        setSkills([]);
      }
